...
private:
  ...
  // track jet variables 
  //(for each gen jet, store the sum of pt of TPs / tracks inside jet cone)
...
void L1TrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( iterL1Track = TTTrackHandle->begin(); iterL1Track != TTTrackHandle->end(); iterL1Track++ ) {
    ...
    if (DebugMode) {
      cout << "L1 track, pt: " << tmp_trk_pt << " eta: " << tmp_trk_eta << " phi: " << tmp_trk_phi << " z0: " << tmp_trk_z0 << " chi2: " << tmp_trk_chi2 << " nstub: " << tmp_trk_nstub;
  ...
  for (iterTP = TrackingParticleHandle->begin(); iterTP != TrackingParticleHandle->end(); ++iterTP) {
    ...
    if (DebugMode) cout << "Tracking particle, pt: " << tmp_tp_pt 
      << " eta: " << tmp_tp_eta 
      << " phi: " << tmp_tp_phi
			<< " z0: "  << tmp_tp_z0 
      << " d0: "  << tmp_tp_d0
      ...
      << " eventID: "   << iterTP->eventId().event()
			...
			<< " tttracks "   << MCTruthTTTrackHandle->findTTTrackPtrs(tp_ptr).size() 
      << endl;
    ...
    if (DebugMode) cout << "TP is associated with "    << nStubTP 
    ...
    if (matchedTracks.size() > 0) {
      ...
      } // end for loop over matched L1 tracks
