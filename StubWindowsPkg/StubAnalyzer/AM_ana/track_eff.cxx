...
track_eff::track_eff(std::string filename, std::string secfilename, std::string outfile, int nevt, float pt_min, float d0_min, bool dbg) :
  ...
  stub_chip(new std::vector<int>), //added
...
{  
  ...
  has_patt = false;
  if (has_patt) cout << "     Using trigger tower definitions" << endl;
  else          cout << "     Using cabling definitions"       << endl;

  m_tilted = true;
  if (m_tilted) cout << "     Using tilted geometry" << endl;
  else          cout << "     Using flat geometry"   << endl;
  ...
  cout << "Success! Track efficiency set up." << endl;
  ...
  //  the void doesn't return true, 
  //  then you dun fucked up.

  if (has_patt) cout << "Success! Converted tower definitions."   << endl;
  else          cout << "Success! Converted cabling definitions." << endl;

  
  track_eff::do_test(nevt); // Launch the test loop over n events
  cout << "Success! Fininished calculating efficiencies." << endl;
}
...
void track_eff::do_test(int nevt) {
  ...
  int ndat = std::min(nevt,static_cast<int>(m_L1TT->GetEntries())); // Automatically truncate nevt to the size of the input file, if needed
  cout << "Main process - calculate efficiencies..." << endl;
  ...
  for (int i=0;i<ndat;++i) {
    ...
    if (m_primaries.size()>300)
      std::cout << "     Found " << m_primaries.size()
                << " interesting particles inducing at least one stub in the event" << i 
                << std::endl;
...
bool track_eff::convert_cabling(std::string cablingfilename) {
  ...
  while (!in.eof()) {
    ...
    // Endcap
      ...
      mod   = 128*detid[9]+64*detid[8]+32*detid[7]+16*detid[6]+8*detid[5]+4*detid[4]+2*detid[3]+detid[2]-1;
    }
    ...
    /* 
      Define rmodid using some other terms
      Take this vector "module" and assign it to the entry of m_modules_to_DTC that corresponds to rmodid
      Now take the value of dtcid and put it into the appropriate spot (for the current rmodid) in the "module" vector
      And finally take that entry that's in "module" and put it in the correct spot in modules_to_DTC.

      Graphical representation where modules_to_DTC was previously given 5 entries.
        m_modules_to_DTC -> [0] [0] [0] [0] [0]
        rmodid = 2
        module = m_modules_to_DTC [0] [***THIS ONE IS NOW MODULE***] [0] [0] [0]
        module = dtcid
        m_modules_to_DTC -> [0] [dtcid] [0] [0] [0]
    */
    ...
    /*
      ...
      dtcid=4
      m_DTC_to_modules [0] [0] [0] [0] [0]
      module = m_DTC_to_modules [0] [0] [0] [***THIS ONE IS NOW MODULE***] [0]
      module = rmodid
      m_DTC_to_modules -> [0] [0] [0] [rmodid] [0]
    */

    // This is a problem if rmodid or dtcid exceeds the value of the space allotted to m_DTC_to_modules or to m_modules_to_DTC.
    // Could perhaps write in an if statement that only continues if
    //     - rmodid<230000 (value allocated to m_modules_to_DTC)
    //    - dtcid<2000 (value allocated to m_DTC_to_modules)
  }
