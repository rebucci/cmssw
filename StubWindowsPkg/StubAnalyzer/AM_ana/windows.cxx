  ...
  for (int j=0;j<n_r;++j) {
    ...
    for (int k=0;k<6;++k) {
      for (int l=0;l<40;++l) {
        ...
        // Loop over all the SW indexes (max is 8*2)
        for (int m=0;m<16;++m) {
          if (k>2) barrel_w[k][rank][m][0] += (fact/(n_mod_tilt[k]*n_lad_tilt[k]))*m_rate_b[k][l][m][2][0];
          else {
            if (rank==0) barrel_w[k][rank][m][0] += (fact/((n_mod_tilt[k]-24)*n_lad_tilt[k]))*m_rate_b[k][l][m][2][0];
            else         barrel_w[k][rank][m][0] += fact/(2*n_lad_tilt[k])*m_rate_b[k][l][m][2][0];
          }
        }
      }
    }
    ...
    for (int k=0;k<5;++k) {
      for (int l=0;l<15;++l) {
        ...
        for (int m=0;m<16;++m) {
          if (k<2) disk_w[k][l][m][0] += fact/(2*n_mod_endcap[l])*m_rate_d[k][l][m][2][0];
          else {
            if (l>=12) continue;
...
void windows::get_effs(int ptype)
{
  ...
  for (int j=0;j<n_e;++j) {
    ...
    for (int j=0;j<10;++j) {
      for (int k=0;k<20;++k) stub_per_lay[j][k]=0;
    } 
    ...
    for (int k=0;k<m_stub;++k) {
      ...
      // Tilted -
      if (m_stub_type->at(k)==1) ladder = 12-m_stub_module->at(k);
      
      // Tilted +      
      if (m_stub_type->at(k)==2) {
...
void windows::print_result(int type)
{
  ...
  for(it1=0;it1<5;it1++) {
    ...
    for(it2=0;it2<15;it2++) {
      ...
      if ((it2<14 && it1<=1) || (it2<11 && it1>1)) cout << ", ";
      else                                         cout << ") )," << endl;
    }
  }//end loop on endcaps
  cout <<")" << endl;
