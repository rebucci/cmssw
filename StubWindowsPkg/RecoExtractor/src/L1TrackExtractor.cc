...
// -----------------------------------------------------------------------------------------------------------
// Tokens
L1TrackExtractor::L1TrackExtractor(edm::EDGetTokenT< edmNew::DetSetVector< TTStub< Ref_Phase2TrackerDigi_ > > > STUB_tag, 
...
void L1TrackExtractor::writeInfo(const edm::Event *event, StubExtractor *stub) 
{
  ...
  if ( TTTrackHandle->size() > 0 )
  {
    ...
    for ( iterTTTrack  = TTTrackHandle->begin(); iterTTTrack != TTTrackHandle->end(); ++iterTTTrack )
    {
      ...
      // Loop over stubs contained in the pattern to recover the info and match them with the stubs in the STUB_extractor container
